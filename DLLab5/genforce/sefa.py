...
def main():
    ...
    weight = weight.flip(2, 3).permute(1, 0, 2, 3).flatten(1).cpu().detach().numpy()
    ...
    e_val, e_vec = np.linalg.eig(weight @ weight.T)
    # e_val, e_vec = torch.linalg.eig(weight @ weight.T)
    # e_val = np.real(e_val)
    # e_val_Index = np.argsort(-e_val)
    # e_vec = torch.from_numpy(e_vec[:, e_val_Index])
    # directions = [e_vec[:, i:i + num_sam].T for i in range(num_sem)]
    directions = torch.from_numpy(e_vec.T)
    ################
